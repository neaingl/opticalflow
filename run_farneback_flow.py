...
def positive_float(value: str) -> float:
    try:
        ...
        raise argparse.ArgumentTypeError("Playback rate must be a number.") from exc
    if val <= 0:
        raise argparse.ArgumentTypeError("Playback rate must be positive.")
    return val
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=positive_float,
        default=1.0,
        help="Output playback rate relative to source FPS (e.g., 0.5 for half-speed/slow motion, 2.0 for faster).",
    )
    return parser
...
def process_video(args: argparse.Namespace) -> None:
    ...
    if fps <= 0:
        fps = 30.0
    output_fps = max(0.1, fps * args.playback_rate)
    ...
    if args.viz in {"hsv", "both"}:
        hsv_writer, hsv_codec = create_writer(hsv_path, output_fps, writer_size)
        print(f"HSV video writer initialized with codec {hsv_codec} -> {hsv_path} (fps={output_fps:.3f})")
    if args.viz in {"quiver", "both"}:
        quiver_writer, quiver_codec = create_writer(quiver_path, output_fps, writer_size)
        print(f"Quiver video writer initialized with codec {quiver_codec} -> {quiver_path} (fps={output_fps:.3f})")
